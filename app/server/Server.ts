...
export class Server {
    ...
    constructor(
        ...
        this.instance = net.createServer((connection: net.Socket) => {
            connection.on('data', (data: Buffer) => this.onDataHandler(connection, data));
            connection.on("close", () => {
              connection.end();
            });  
          });
        ...
        if (commandData.type === DataType.Array) {
            ...
            switch(command.value.toLowerCase()) {
                ...
                if (isString(keyData) && keyData.value) {
                    ...
                    this.storage.set(keyData.value, valueData, expirationMs);
                }
                ...
                if (isString(subCmdData) && subCmdData.value?.toLowerCase() === Commands.GET_CMD && isString(keyData)) {
                    switch (keyData.value?.toLowerCase()) {
                    case Commands.CONFIG_DIR_CMD: 
                        reply = this.encoder.encode([Commands.CONFIG_DIR_CMD, this.args.dir]);
                        break;
                    case Commands.CONFIG_DB_FILENAME_CMD:
                        reply = this.encoder.encode([Commands.CONFIG_DB_FILENAME_CMD, this.args.dbfilename]);
                        break;
                    }              
                }            
                break;
                ...
                if (isString(subCmdData) && subCmdData.value === Commands.INFO_REPLICATION_CMD) {
                    const role = !this.args.replicaof ? 'master' : 'slave';
                    const info = [
                    `role:${role}`,      
                ...
                if (isString(subCmdData)) {
                    ...
                    case Commands.REPLCONF_LISTENING_PORT_CMD: {
                        ...
                        this.listeningPorts.push(Number(listeningPort.value));
                        break;
                    }
